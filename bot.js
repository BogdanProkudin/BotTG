...
bot.onText(/\/ДобавитьКатегорию/, async (msg) => {
  ...
  try {
    ...
    await bot.sendMessage(
      chatId,
      "Укажите в какую категорию хотите добавить товар",
      {
        reply_markup: {
          keyboard: [
    ...
    if (user && user.isInProcess && user.processType === "add_category") {
      if (text !== "Назад") {
        ...
        await collectionUser.updateOne(
          ...
          {
            $set: {
              processType: "add_category_item_photo",
...
bot.on("photo", async (msg) => {
  ...
  if (
    ...
    await collectionUser.updateOne(
      ...
      {
        $set: {
          processType: "add_category_item_photo_description",
          addCategoryItemPhoto: photo,
        },
      }
    );
    await bot.sendMessage(
...
bot.on("message", async (msg) => {
  ...
  try {
    ...
    if (
      ...
      await bot.sendMessage(
        chatId,
        "Спасибо! Вы согласились с политикой конфиденциальности",
        {
          reply_markup: {
            keyboard: [["Назад"]],
          },
    ...
    if (
      ...
      bot
        .sendDocument(chatId, "./ПолитикаКондифициальности.docx", {
    ...
    if (
      ...
      await bot.sendMessage(chatId, "Вы вернулись к выбору", {
        reply_markup: {
    ...
    if (
      ...
      if (text !== "Назад") {
        ...
        const currentCategoryItems =
          currentCategory.categoryItem[currentCategory.categoryItem.length - 1]
            .itemId;
    ...
    if (text !== "Назад" && user.step === "waitForNewCategoryName") {
      try {
        ...
        await bot.sendMessage(
          chatId,
          "Название успешно обновлено. Вы вернулись в главное меню",
          {
            reply_markup: {
              keyboard: [
                ["Онлайн-витрина", "Наш каталог"],
    ...
    if (text === "Онлайн-витрина" && !user.isInProcess) {
      ...
      await bot.sendMessage(
        chatId,
        "Согласны ли вы с нашей политикой конфиденциальности?",
        {
          reply_markup: {
            keyboard: [["Да"], ["Ознакомится"], ["Нет"]],
          },
...
`;
    ...
    if (
      ...
      await bot.sendMessage(
        chatId,
        `Вы выбрали каталог: ${isFirst.categoryInfo.name}`,
        {
          reply_markup: {
            keyboard: [["Назад"]],
            resize_keyboard: true,
    ...
    if (
      ...
      await bot.sendMessage(
        chatId,
        `Вы выбрали каталог: ${isSecond.categoryInfo.name}`,
        {
          reply_markup: {
            keyboard: [["Назад"]],
            resize_keyboard: true,
    ...
    if (
      ...
      await bot.sendMessage(
        chatId,
        `Вы выбрали каталог: ${isThird.categoryInfo.name}`,
        {
          reply_markup: {
            keyboard: [["Назад"]],
            resize_keyboard: true,
      ...
      await bot.sendMessage(
        chatId,
        `Вы выбрали каталог: ${isFourth.categoryInfo.name}`,
        {
          reply_markup: {
            keyboard: [["Назад"]],
            resize_keyboard: true,
  ...
  try {
    ...
    if (callback_data.includes("update_category")) {
      try {
        ...
        const foundedItem = foundedCategory.categoryItem.find(
    ...
    if (vitrinaData.price === "4") {
      ...
      const photo = await selectedProduct.categoryItem.find(
        (el) => el.itemId === vitrinaData.id
      ).photo;
