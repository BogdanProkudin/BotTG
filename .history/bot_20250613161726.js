...
bot.onText(/\/ДобавитьКатегорию/, async (msg) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const chatType = msg.chat.type; // 'private', 'group', 'supergroup'
  if (chatType === 'supergroup') {
  ...
  try {
    ...
    await bot.sendMessage(chatId, 'Укажите в какую категорию хотите добавить товар', {
      reply_markup: {
        keyboard: [
...
bot.onText(/\/ИзменитьКатегорию/, async (msg) => {
  ...
  try {
    ...
    await bot.sendMessage(chatId, 'Укажите какую категорию хотите изменить', {
      reply_markup: {
        keyboard: [
          [slidesFor4k.categoryInfo.name, slidesFor7k.categoryInfo.name],
          [slidesFor14k.categoryInfo.name, slidesFor10k.categoryInfo.name],
  ...
  try {
    ...
    if (user && user.isInProcess && user.processType === 'add_category') {
      if (text !== 'Назад') {
        ...
        if (!selectedCategory) {
          await bot.sendMessage(chatId, 'Такая категория не существует', {
            reply_markup: {
              keyboard: [['Назад']],
              resize_keyboard: true,
              one_time_keyboard: true,
        ...
        await bot.sendMessage(chatId, 'Отправьте фотографию товара', {
          reply_markup: {
            keyboard: [['Назад']],
            resize_keyboard: true,
            one_time_keyboard: true,
...
bot.on('photo', async (msg) => {
  ...
  if (user && user.isInProcess && user.processType === 'add_category_item_photo') {
    ...
    await bot.sendMessage(
      chatId,
      'Фото успешно Добавлено. Теперь отправьте новое название. Внимание! В названии должна быть цена в таком формате:Цена: 2000 ₽',
      {
        reply_markup: {
          keyboard: [['Назад']],
          resize_keyboard: true,
  ...
  if (user && user.isInProcess && user.step === 'getCategoryPhoto') {
    ...
    await bot.sendMessage(
      chatId,
      'Фото успешно обновлено. Теперь отправьте новое название. Внимание! В названии должна быть цена в таком формате:Цена: 2000 ₽'
    );
    await collectionUser.updateOne(
      { userId },
      {
...
if(user && user.isInProcess && user.processType === 'rules' && text === "Ознакомится"){
   const chatId = msg.chat.id;
       ...
       await bot.sendMessage(
        chatId,
        'Политика обработки персональныхданных


НастоящаяПолитика обработки персональных данных (далее – Политика) относится к сайту с
доменным именем florimondi.ru и чатботу расположенному на интернет ресурсе Telegram Messenger по адресу @Flori_Mondi_Bot
(далее – Сайт и чат бот) и составлена в соответствии с требованиямиФедерального закона от 27.07.2006. №152-ФЗ «О персональных данных» (далее -
Закон о персональных данных) с целью определения порядка обработки персональныхданных и мер по обеспечению безопасности персональных данных, предпринимаемых Индивидуальным
предпринимателем Карпуниным Андреем Анатольевичем ИНН: 771971874638 ОГРНИП:320774600392892 (далее-Оператором)

 Настоящая Политика разработана с учетом
требований Конституции Российской Федерации, а также в соответствии с Законом,иными федеральными законами и нормативными актами Российской Федерации,
определяющими порядок обработки персональных данных, обеспечения безопасности иконфиденциальности такой информации в целях обеспечения защиты прав и свобод
человека и гражданина при обработке его персональных данных Оператором.
Положениянастоящей Политики являются обязательными для исполнения работниками Оператора,
имеющими доступ к персональным данным.
Положениянастоящей Политики могут быть в любое время изменены Оператором в одностороннем
порядке.
ИспользованиеСайта и чат бота лицами, имеющими доступ к Сайту и чат боту, посредством сети
Интернет (далее – Пользователями) означает согласие с настоящей Политикой иусловиями обработки персональных данных пользователей. В случае несогласия с
условиями Политики Пользователь должен прекратить использование Сайта и чатбота.


Настоящая
Политика применяется только к Сайту и чат боту. Администрация Сайта неконтролирует и не несет ответственность за сайты третьих лиц, на которые
Пользователь может перейти по ссылкам, доступным на Сайте.

1.Персональные данные обрабатываются Оператором в целях:

·
предоставления доступа Пользователю ксервисам, информации и/или материалам, содержащимся на Сайте.
·
продажи товаров и услуг, исполнениядоговоров купли-продажи, аутентификацию и подтверждение действий пользователя,
индивидуальный учёт клиентских заказов, обработку и доставку заказов, для повышениекачества работы и оказываемых услуг, информирование о специальных предложениях
и акциях.
·       защиты жизни, здоровья и иных жизненно
важных интересов субъектов персональных данных;
·       в иных законных целях.

2.
Обработка персональных данных осуществляется Оператором на основе следующихпринципов:
·
законности целей и способов обработкиперсональных данных, добросовестности и справедливости действий Оператора и
недопущения Оператором бездействия;
...
достаточности для целей обработки, недопустимости обработки персональныхданных, избыточных по отношению к целям, заявленным при сборе персональных
данных;
...
·
соответствия содержания и объемаобрабатываемых персональных данных целям их обработки;
·
...
позволяющей определить субъекта персональных данных, не дольше, чем этоготребуют цели обработки персональных данных, если срок хранения персональных
...
·
имя, фамилия, отчество, номера телефонов,адреса электронной почты, адреса для доставки товаров или оказания услуг,
реквизиты компании, которую представляет пользователь, должность в компании,которую представляет пользователь, аккаунты ',
        {
          reply_markup: {
            keyboard: [
            ['Назад'],
    ...
    if (user && user.isInProcess && user.processType === 'add_category_item_photo_description') {
      if (text !== 'Назад') {
        ...
        if (!currentCategory) {
          await bot.sendMessage(chatId, 'Категория не найдена', {
            reply_markup: {
              keyboard: [['Назад']],
              resize_keyboard: true,
              one_time_keyboard: true,
        ...
        await bot.sendMessage(chatId, 'Товар успешно добавлен в категорию', {
          reply_markup: {
            keyboard: [['Назад']],
            resize_keyboard: true,
            one_time_keyboard: true,
    ...
    if (text !== 'Назад' && user.step === 'waitForNewCategoryName') {
      try {
        ...
        if (!category) {
          await bot.sendMessage(chatId, 'Категория не найдена. Попробуйте еще раз.', {
            reply_markup: {
              keyboard: [['Назад']],
              resize_keyboard: true,
              one_time_keyboard: true,
        ...
        await bot.sendMessage(chatId, 'Название успешно обновлено. Вы вернулись в главное меню', {
          reply_markup: {
            keyboard: [
              ['Онлайн-витрина', 'Наш каталог'],
              ['О нас', 'Мы на карте', 'Наш сайт'],
        ...
        await bot.sendMessage(
          chatId,
          'Произошла ошибка при обновлении категории. Попробуйте еще раз.',
          {
            reply_markup: {
              keyboard: [['Назад']],
              resize_keyboard: true,
...
`;
    ...
    if (isThird !== null && isThird !== undefined && user.processType === 'catalog') {
      ...
      await bot.sendMessage(chatId, `Вы выбрали каталог: ${isFourth.categoryInfo.name}`, {
        reply_markup: {
          keyboard: [['Назад']],
          resize_keyboard: true,
          one_time_keyboard: true,
  ...
  try {
    ...
    if (callback_data.includes('update_category')) {
      try {
        ...
        bot.sendMessage(
          chatId,
          `Вы выбрали товар: ${foundedItem.caption}\n\nТеперь отправьте новое фото `
        );
        await collectionUser.updateOne(
