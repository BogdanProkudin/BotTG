...
`;
    ...
    if (text === 'Монобукеты' && user.processType === 'catalog') {
      ...
      async function sendCatalog() {
        const slidesFor4kk = await collectionCategory.findOne({ categoryInfo.id":1} });
        console.log(slidesFor4kk, '1213');
        const slidesFor4K = await slidesFor4k.forEach(async (item, index) => {
