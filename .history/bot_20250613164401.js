...
·
продажи товаров и услуг, исполнения договоров купли-продажи, аутентификацию и подтверждение действий пользователя,
индивидуальный учёт клиентских заказов, обработку и доставку заказов, для повышение  качества работы и оказываемых услуг, информирование о специальных предложениях
и акциях.
...
·
законности целей и способов обработки персональных данных, добросовестности и справедливости действий Оператора и
...
достаточности для целей обработки, недопустимости обработки персональных данных, избыточных по отношению к целям, заявленным при сборе персональных
данных;
  ...
  Сообщение: ${text}`;
    ...
    if (text === 'Назад' && user.processType !== 'finished') {
      ...
      if (
        ...
        return;
      } else if(user && user.processType !== "rules_text") {
        log
        const message = await cancelProcess(userId, collectionUser);
        const user = await collectionUser.findOne({ userId });
        if (!user) {
...
if(user && user.isInProcess && user.processType === 'rules' && text === "Ознакомится"){
  ...
  caption: 'Документ с нашей политикой',
...
if(user && user.isInProcess && user.proccesType === "rules_text" && text === "Назад"){
    const chatId = msg.chat.id;

       await bot.sendMessage(
        chatId,
        'Вы вернулись на выбор',
        {
          reply_markup: {
            keyboard: [
            ['Да',"Нет","Ознакомится"],
