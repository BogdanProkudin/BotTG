...
`;
      ...
      await bot.sendMessage(chatId, 'Выберете интересующий вас каталог ', {
        reply_markup: {
          keyboard: [
            ...
            ['Раскидистые букеты', slidesFor10k.categoryInfo.name],
            ['Назад'],
          ],
          resize_keyboard: true,
          one_time_keyboard: true,
    ...
    const isFouth = await collectionCategory.findOne({
      'categoryInfo.id': '4',
      'categoryInfo.name': text,
    });
    ...
    if (isThird !== null && isThird !== undefined && user.processType === 'catalog') {
      ...
      await sendCatalog();
    } else if (isFourth !== null && isFouth !== undefined && user.processType === 'catalog') {
      await collectionUser.updateOne(
        { userId },
        { $set: { isInProcess: true, processType: 'catalog_price=10000++' } }
      );
