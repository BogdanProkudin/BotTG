...
bot.on('callback_query', async (query) => {
  try {
    ...
    if (vitrinaData.price === '4') {
      console.log(vitrinaData, 'DATTAA');
      const selectedProduct = await collectionCategory.findOne({
        'categoryItem.itemId': vitrinaData.id,
      });
      console.log(selectedProduct, '!!!');
      console.log(selectedProduct.categoryItem.find((el) => el.id === vitrinaData.id));
      const now = new Date();
      const year = now.getFullYear();
      const month = now.getMonth();
      ...
      const price = await selectedProduct.categoryItem .find((el) => el.id === vitrinaData.id)
