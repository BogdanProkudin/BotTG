...
bot.onText(/\/updateCategory/, async (msg) => {
  ...
  try {
    ...
    const slidesFor14k = await collectionCategory.findOne({ 'categoryInfo.id': '4' });

    await collectionUser.updateOne(
      { userId },
      { $set: { isInProcess: true, step: 'getCategoryName', categoryProcessType: 'add' } },
      { upsert: true }
    );
...
`;
    ...
    if (isThird !== null && isThird !== undefined && user.processType === 'catalog') {
      ...
      async function sendCatalog() {
        const slidesFor10moreK = await collectionCategory.findOne({ 'categoryInfo.id': '4' });
