...
`;
    ...
    if (text === 'Монобукеты' && user.processType === 'catalog') {
      ...
      async function sendCatalog() {
        const slidesFor4kl = await collectionCategory.findOne({ 'categoryInfo.id': '1' });
        console.log(slidesFor4kl, '1');

        await slidesFor4kl.categoryItem.forEach(async (item, index) => {
